...
static void syntaxError(char* message) {
	fprintf(listing, "\n>>> ");
	fprintf(listing, "Áõ¿¡»Ô 2020141460029 Syntax error at line %d: %s", lineno, message);
	Error = TRUE;
}
...
static void match(TokenType expected) {
	if (token == expected) {
		...
		fprintf(listing, "\n>>> ");
		fprintf(listing, "Áõ¿¡»Ô 2020141460029 Syntax error at line %d: unexpected token ->", lineno);
